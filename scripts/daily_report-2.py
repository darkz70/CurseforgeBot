...
def fetch_download_count(slug: str, project_id: int | None = None) -> dict:
    """Получает точные данные через CFWidget API — без ключа, без округления."""
    # CFWidget — публичный API с точными данными
    try:
        resp = requests.get(
            CFWIDGET_API.format(slug=slug),
            headers={"User-Agent": "CurseforgeBot/1.0 (github.com/darkz70/CurseforgeBot)"},
            timeout=20,
        )
        ...
        if isinstance(downloads, dict):
            count = downloads.get("total", 0)
        elif isinstance(downloads, (int, float)):
            count = int(downloads)
        else:
            count = 0
        name = data.get("title", slug)
        print(f"CFWidget: {name} = {count:,} скачиваний")
    ...
    if abbr_match:
        ...
        if ld_match:
            ...
            if dl_match:
                ...
                raise ValueError(f"Не удалось найти число скачиваний: {url}")
